...
module Message::Elimination
	def self.start_elimination(current_episode, shuffled_list, immune_contestant)
		...
		status = {
			...
			eliminated: {
				en: "ELIMINATED",
				es: "ELIMINAD#{shuffled_list[-1].gender[:suffix][:es].upcase}"
			}
		}
		...
		description2 = {
			en: "#{immune_contestant.name} won immunity last time, so #{immune_contestant.gender[:pronoun1][:en]}'s safe",
			es: "#{immune_contestant.name} ganó la inmunidad la vez pasada, así que, #{immune_contestant.gender[:pronoun1][:es]} está a salvo"
		}
		indication = {
		...
		description3 = {
			...
			es: "¡#{shuffled_list[-1].name} ha sido eliminad#{shuffled_list[-1].gender[:suffix][:es]} por los espectadores y será tratad#{shuffled_list[-1].gender[:suffix][:es]} con TLC!"
		}
		indication2 = {
